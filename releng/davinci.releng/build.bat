...
@rem
set MAQETTA_BUILD_DIR=J:\obuild
...
if exist %MAQETTA_BUILD_DIR%\repository\weblabs\.git (
    @rem "Doing 'git pull'..."
    cd /D %MAQETTA_BUILD_DIR%\repository\weblabs
    git pull
) else (
    @rem "Cloning repository. This may take a few moments..."
    cd /D %MAQETTA_BUILD_DIR%\repository
	echo current dir  %cd%
	echo start cloning  %gitRepository%
...
java -Ddeployment-type=internal -DmaqettaCode=D:\MyProjects\maqetta -DbuilderDir=D:\MyProjects\maqetta\releng\davinci.releng\orion\builder -Dtimestamp=20150323 -DbuildDirectory=D:\MyProjects\maqetta\build -jar %baseLocation%\plugins\org.eclipse.equinox.launcher_%launcherVersion%.jar -application org.eclipse.ant.core.antRunner -buildfile %relEngDir%\orion\buildAll.xml 